...
class TestFollowUserView(TestCase):
    ...
    def test__follow_user__logged_user__successfully(self):
        self.client.login(email=self.credentials1['email'], password=self.credentials1['password'])

        response = self.client.post(reverse('follow-user', kwargs={'pk': self.user2.pk}))
    ...
    def test__follow_user__anonymous_user__expected_redirect_to_login(self):
        ...
        self.assertRedirects(response, reverse('login') + '?next=%2Faccounts%2F2%2Ffollow%2F')